...
namespace TradeFutNight.Views.Prefix9
{
    ...
    /// U_90002.xaml 的互動邏輯
    /// </summary>
    public partial class U_90002 : UserControlParent, IViewSword
    {
        private U_90002_ViewModel _vm;

        public U_90002()
        {
            InitializeComponent();
            _vm = (U_90002_ViewModel)DataContext;
        }
        ...
        public async Task Save()
        {
            ...
            {
                var trackableData = _vm.MainGridData.CastToIChangeTrackableCollection();
                var domainData = _vm.MapperInstance.Map<IList<TXN>>(trackableData.DeletedItems);
                ...
                using (var das = Factory.CreateDalSession())
                {
                    ...
                    try
                    {
                        ...
                        {
                            dJSW.Delete(item.TXN_ID);
                            DbLog($"刪除{item.TXN_ID}的JSW設定", das);

                            dUTP.Delete(item.TXN_ID);
                            DbLog($"刪除{item.TXN_ID}的UTP設定", das);
                        }
